  ...
  useEffect(() => {
    async function fetchData() {
      ...
      if (ordersData.length > 0) {
        ...
        const avgOrderValue = Math.round(totalRevenue / ordersData.length);
  ...
  return (
    <div className="space-y-8">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard title="Всего заказов" value={stats.totalOrders.toString()} icon="📦" />
        <StatCard title="Выручка" value={`${stats.totalRevenue.toLocaleString("ru-RU")} ₸`} icon="💰" />
        <StatCard title="Средний чек" value={`${stats.avgOrderValue.toLocaleString("ru-RU")} ₸`} icon="📊" />
        ...
        <div className="bg-white p-4 rounded-lg shadow lg:col-span-2">
          ...
          <ResponsiveContainer width="100%" height={250}>
            <LineChart data={ordersByDate}>
              ...
              <YAxis tick={{ fontSize: 12 }} />
              <Tooltip />
              <Line type="monotone" dataKey="orders" stroke="#82ca9d" strokeWidth={2} dot={{ fill: "#82ca9d" }} />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </div>
            ...
            <tbody className="divide-y divide-gray-200">
              {orders.slice(0, 10).map((order) => (
                <tr key={order.id} className="hover:bg-gray-50">
                  <td className="px-4 py-3 text-sm text-gray-500">{order.order_id}</td>
                  ...
                  <td className="px-4 py-3 text-sm font-medium text-gray-900">{order.total_amount.toLocaleString("ru-RU")} ₸</td>
              ...
              {orders.length === 0 && (
                <tr>
                  <td colSpan={6} className="px-4 py-8 text-center text-gray-500">Нет данных о заказах</td>
                </tr>
              )}
            </tbody>
          </table>
